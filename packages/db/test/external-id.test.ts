...
describe('0004 down → up (peeling 0006 off first)', () => {
  ...
  it('down restores 0003’s uuid-typed function exactly, then up restores the text one', async () => {
    // 0006 sits on top of 0004 now, so one step down no longer lands on 0004. Peel it
    ...
    expect(await migrate('up', OWNER_URL)).toEqual(['0004_users_external_id', '0006_outbox']);
