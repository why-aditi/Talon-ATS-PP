...
12. **Tests never touch the development database.** Every suite opens a **dedicated** database, created on demand. A test suite that drops `public` on the dev database will eventually do it while someone is mid-demo — this one did, which is why the rule exists.

    **Testcontainers is the target and is NOT yet in force.** M0a runs against the docker-compose Postgres on a separate `talon_test`; the deviation is recorded in `packages/db/test/urls.ts` and tracked in #36. Stated here rather than asserted as done, because a control the doc claims and CI doesn't run is worse than a known gap. Until #36 lands, **that database persists between runs** — a suite can observe state an earlier run left behind, which is exactly how #31 reached us.
