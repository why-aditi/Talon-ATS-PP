...
4. **Esc mid-drag.** Needs no rollback at all: nothing is mutated while a drag is in flight, so there is no state to restore. That is stronger than converging a cancel path onto the failure path. (An earlier draft of this spec described a board reducer with a shared `revert(snapshot)`; the implementation uses TanStack's `onError` snapshot restore and Esc needs neither. Corrected here rather than left describing code that does not exist.)
...
## 8d. Review findings on PR #10

The `reviewer` agent found three blocking defects. All three were real; two of them falsified claims made in §8b.

1. **`neighboursFor` read "over is the dragged card itself" as "append to end."** dnd-kit reports the dragged card as its own `over` on every pickup and on any drop that did not move — and the card is filtered out of the candidate list, so `indexOf` missed and the function reported the bottom of the column. Two consequences: the pickup announcement was still overwritten for **6 of the 9 cards** (§8b item 2 claimed this fixed — it was verified on Ana Petrova, the single card in her column, which is the one case where the guard accidentally works), and Space-Space relocated a card and fired a PATCH. Fixed, and covered by a test that walks **every** card in a four-card column.

2. **An always-on sort silently discarded every reorder.** `applyFilters` sorted by dwell on every render, so a drag-reorder was spliced in optimistically, confirmed by the server, and then sorted straight back out — the PATCH fired and the board snapped back. Invisible only because the fixture is already in descending-dwell order. A sort and a hand-arranged column are mutually exclusive: `manual` is now a sort option and the only one under which a within-column drop is accepted; any other sort refuses it and says so. Cross-stage moves are unaffected. Also fixed here: `handleDragEnd` and the announcements now read the **rendered** columns, not the raw cache, so indices match what the user actually dropped onto.

3. **Motion values were hard-copied as literals and reduced motion never reached the drag.** `duration: 200` and the spring curve were `motion.duration.base` and `motion.easing.spring` verbatim; because a `dropAnimation` is a JS prop rather than a `className`, `token-usage.test.ts` structurally could not see them. They are read from `TOKENS` now. Separately, the drop animation runs through the Web Animations API and the sortable's `transition` is an **inline** style — neither is reached by the `prefers-reduced-motion` block in `globals.css`, so both now ask directly.

Also fixed from the same review: `fromStageId` moved into `MoveStageBodySchema` (it was bolted on locally with `.extend()`, so migrating the schema as-is would have handed the api stream a body that 400s the real client); every handler response is validated, not just the GET; conflict scenarios no longer re-arm on each refetch; a failed **reorder** now shows the same banner as a failed move; the recruiter filter is disabled rather than inert (the recruiter belongs to the job, so on a job-scoped board it can only match all or none); `beforeId` precedence is stated in the contract; the drop announcement carries its position; and the failure banner is `aria-live="off"` so it cannot become a second live region.

Tests that would have passed with the behaviour broken were rewritten: the "4 / 2 / 1 / 1 / 1" test asserted that a helper *function* was truthy and never checked a count; the loading test asserted only half its own name; the live-region test used `toBeLessThanOrEqual(1)`, which also passes at zero. `pipeline-rollback.test.tsx` is new and covers non-negotiable #14 directly — it captures the cache's whole sequence, so it fails both if the card never moves and if it is not put back.
...
13. `boardCoordinateGetter` reaches a column with **no cards in it**, moves back the other way, does not wrap at the ends, and orders by column position rather than DOM order. It deliberately does **not** skip terminal columns — they are reachable so the refusal can be announced (§8b item 5).
14. `neighboursFor` places a card moving *down* within its column after the card it landed on — the off-by-one a raw index produces, which would make the move a silent no-op — and treats "over is the dragged card itself" as staying put rather than appending (§8d item 1).
