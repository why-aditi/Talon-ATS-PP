...
**Interviews per week:** eight columns, height proportional to count, value above, week label below. All bars `bg-chart-bar-idle` except the last, `bg-chart-bar-current` — added in PR A, measured — see OQ-6. Rendered as a flex row of divs — five bars and eight columns of known height do not justify a charting dependency.

All three panels get `role="img"` with an `aria-label` summarising the series, and a visually-hidden table of the same numbers. A bar chart that only exists as coloured divs is invisible to a screen reader (§4.15).
